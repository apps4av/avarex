...
class PlateProfileController {
  PlateProfileController({
  ...
  String? get selectedProcedure => _selectedProcedure;
  bool get isVisible => _isVisible;

  void initFromSettings() {
    _isVisible = Storage().settings.isPlateProfileVisible();
    _lastPlateName = Storage().currentPlate;
  }

  void syncForPlate(String currentPlate) {
    if (_lastPlateName != currentPlate) {
      _isVisible = false;
      Storage().settings.setPlateProfileVisible(false);
      _points.clear();
      _profileKey = "";
      _selectedProcedure = null;
      _lastPlateName = currentPlate;
    }
  }

  void updateSelectedProcedure(List<String> procedureNames) {
    ...
    final String cacheKey = "$airport|${Storage().currentPlate}|$_selectedProcedure";
    if (cacheKey != _profileKey) {
      _profileKey = cacheKey;
      _points.clear();
      if (_selectedProcedure != null) {
        _loadProfile(_selectedProcedure!);
      }
    }
  }

  void selectProcedure(String value) {
    _selectedProcedure = value;
    _profileKey = "${Storage().settings.getCurrentPlateAirport()}|"
  ...
  void setProfileFromProcedure(String value) {
    _selectedProcedure = value;
    _profileKey = "${Storage().settings.getCurrentPlateAirport()}|"
        "${Storage().currentPlate}|$value";
    _points.clear();
  ...
  void addProcedureToPlan(String value, BuildContext context) {
  ...
  void toggleVisibility() {
    ...
    Storage().settings.setPlateProfileVisible(_isVisible);
    if (!_isVisible) {
      _points.clear();
      _profileKey = "";
      onRebuild();
    ...
    onRebuild();
    if (_selectedProcedure != null && _points.isEmpty) {
      _loadProfile(_selectedProcedure!);
    }
  }

  Widget buildOverlay(BuildContext context, ValueNotifier notifier) {
    if (!_isVisible || _selectedProcedure == null || _points.isEmpty) {
      return Container();
    }
  ...
  Future<void> _loadProfile(String procedureName) async {

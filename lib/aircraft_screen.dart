...
class AircraftScreenState extends State<AircraftScreen> {
  ...
  Widget _makeBody(List<Aircraft>? items) {
    ...
    List<_Entry> entries = [];
    entries.add(_Entry("Tail Number",                               "tail",           active.tail,           const Tooltip(message: "Tail number, for example, N172EF.", child: Icon(Icons.info))));
    entries.add(_Entry("Type",                                      "type",           active.type,           const Tooltip(message: "Type, for example, C172.", child: Icon(Icons.info))));
    ...
    entries.add(_Entry("PIC",                                       "pic",            active.pic,            const Tooltip(message: "Name of the pilot, for example, John Smith.", child: Icon(Icons.info))));
    entries.add(_Entry("PIC Information",                           "picInfo",        active.picInfo,        const Tooltip(message: "Pilot information, for example, phone number.", child: Icon(Icons.info))));
    entries.add(_Entry("Home Base",                                 "base",           active.base,           const Tooltip(message: "Where the aircraft is based at, for example, KBVY.", child: Icon(Icons.info))));
    entries.add(_Entry("Mode S Code",                               "icao",           active.icao,           const Tooltip(message: "Mode S code in base 16 / hex (registry.faa.gov), for example A12105.", child: Icon(Icons.info))));
    entries.add(_Entry("Cruise Speed",                              "cruiseTas",      active.cruiseTas,      const Tooltip(message: "Airspeed in cruise in unit Knots, for example, 110.", child: Icon(Icons.info))));
    entries.add(_Entry("Fuel Endurance",                            "fuelEndurance",  active.fuelEndurance,  const Tooltip(message: "Fuel endurance in hours, for example, for 5 hours 30 minutes enter 5.5.", child: Icon(Icons.info))));
    entries.add(_Entry("Fuel Burn Rate",                            "fuelBurn",       active.fuelBurn,       const Tooltip(message: "Fuel burn rate in unit per hour, for example, for 10 gallons per hour, enter 10.", child: Icon(Icons.info))));
    entries.add(_Entry("Sink Rate",                                 "sinkRate",       active.sinkRate,       const Tooltip(message: "Sink rate in unit feet per minute, for example, 700.", child: Icon(Icons.info))));
    entries.add(_Entry("Wake Turbulence",                           "wake",           active.wake,           const Tooltip(message: "Wake turbulence category, enter one of LIGHT, MEDIUM, or HEAVY.", child: Icon(Icons.info))));

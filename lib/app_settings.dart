...
class AppSettings {
  ...
  List<String> getLayers() {
    return (Settings.getValue("key-layers-v32", defaultValue: "Nav,Chart,OSM,Weather,NOAA-Loop,TFR,Plate,Traffic,PFD,Tracks") as String).split(",");
  }

  List<bool> getLayersState() {
    return (Settings.getValue("key-layers-state-v32", defaultValue: "true,true,true,false,false,true,false,true,false,false") as String).split(",").map((String e) => e == 'true' ? true : false).toList();
  }

  void setLayersState(List<bool> state) {
    Settings.setValue("key-layers-state-v32", state.map((bool e) => e.toString()).toList().join(","));
  }
  ...
  void setChecklist(String name) {
    Settings.setValue("key-user-checklist", name);
  }

  String getChecklist() {
    return Settings.getValue("key-user-checklist", defaultValue: "") as String;
  }
